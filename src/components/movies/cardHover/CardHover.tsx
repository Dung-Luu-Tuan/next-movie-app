...
const CardHover = ({ slideRect, detail }: any) => {
  ...
  useEffect(() => {
    const itemChecking = movies?.find(
      (movie: any) => movie?.data?.movie?._id === detail._id
    );
    if (!itemChecking) {
      ...
      setMovieDetail(itemChecking.data.movie);
    }
  ...
  return createPortal(
    <Link href={`movies/${movieDetail.slug}`}>
      <Card
        component="a"
        target="_blank"
        ...
        style={{
          ...
          height: slideData?.height + 90 || 0,
          marginLeft: -25,
          zIndex: 999,
        }}
      >
        <Card.Section>
          <Image src={movieDetail.thumb_url} h={160} alt="img" />
        </Card.Section>

        <Text className={classes.name}>{movieDetail.name}</Text>
        ...
        <Text className={classes.continue}>Xem thêm &#62;</Text>
      </Card>
    </Link>,
    portalContainer
  );
